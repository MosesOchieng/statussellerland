...
function Footer({ onAction }: { onAction: (message: string) => void }) {
  return <footer className="bg-[#192541] px-5 pb-8 pt-14 text-[#f6f2e8] lg:px-10"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-9 border-b border-white/10 pb-12 md:flex-row"><div><Logo dark /><p className="mt-5 max-w-xs text-sm leading-6 text-white/50">The fastest way to sell on social media. Built for the businesses moving East Africa forward.</p></div><div className="grid grid-cols-2 gap-x-16 gap-y-8 text-sm sm:grid-cols-3"><FooterGroup title="Explore" links={['How it works', 'For businesses', 'Pricing', 'FAQ']} onAction={onAction} /><FooterGroup title="Company" links={['About us', 'Stories', 'Contact']} onAction={onAction} /><div><p className="font-bold text-white">Stay in the loop</p><p className="mt-3 max-w-[150px] text-xs leading-5 text-white/45">Small notes for big moves.</p><button onClick={() => onAction('Newsletter signup coming your way')} className="mt-3 flex items-center gap-2 text-xs font-bold text-[#16b878]" data-testid="button-newsletter">Join the list <ArrowRight className="h-3 w-3" /></button></div></div></div><div className="flex flex-col justify-between gap-3 pt-7 text-xs text-white/35 sm:flex-row"><span>© 2024 StatusSeller Technologies</span><span>Made for the ones making it happen.</span></div></div></footer>;
}
...
function Home() {
  ...
  useEffect(() => {
    ...
    return <main className="page-shell grain min-h-[100dvh]"><Nav onAction={notify} /><Hero onAction={(message) => { if (message.includes('Demo')) setShowDemo(true); else notify(message); }} /><SocialSellingSection /><HowItWorks /><BusinessSection onAction={notify} /><AppSection /><Waitlist /><FAQ /><Stories /><Pricing onAction={notify} /><CTA onAction={notify} /><Footer onAction={notify} />{notice && <Notice message={notice} onClose={() => setNotice(null)} />}{showDemo && <DemoModal onClose={() => setShowDemo(false)} />}</main>;
}
