...
function Home() {
  ...
  useEffect(() => {
    ...
    return <main className="page-shell grain min-h-[100dvh]"><Nav onAction={notify} /><Hero onAction={(message) => { if (message.includes('Demo')) setShowDemo(true); else notify(message); }} /><SocialSellingSection /><HowItWorks /><BusinessSection onAction={notify} /><AppSection /><Waitlist /><Stories /><Pricing onAction={notify} /><CTA onAction={notify} /><Footer onAction={notify} />{notice && <Notice message={notice} onClose={() => setNotice(null)} />}{showDemo && <DemoModal onClose={() => setShowDemo(false)} />}</main>;
}
