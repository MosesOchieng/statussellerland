...
function Pricing({ onAction }: { onAction: (message: string) => void }) {
  ...
  const [currency, setCurrency] = useState<'KES' | 'USD' | 'NGN' | 'ZAR'>('KES');
  const currencies = { KES: { symbol: 'KSh', values: [999, 2499, 5999] }, USD: { symbol: '$', values: [7.5, 18.5, 44.5] }, NGN: { symbol: '₦', values: [12500, 31000, 74500] }, ZAR: { symbol: 'R', values: [180, 450, 1080] } };
  const planNames = ['Starter', 'Growth', 'Pro'];
  const planColors = ['#16b878', '#4d8bff', '#a25dd1'];
  const planDescriptions = ['Businesses starting to sell through social media', 'Businesses already getting regular social orders', 'Businesses ready to scale their social sales'];
  const features: [string, string, string, string][] = [
    ['Turn a Status into a Store', '✓', '✓', '✓'], ['Product Shopping Popup', 'Customers can view products, prices, variants & availability', '✓', '✓'], ['Add to Cart', 'Customers can build an order directly from your post', '✓', '✓'], ['Buy Now', 'Customers can purchase immediately', '✓', '✓'], ['Automatic Checkout', 'Customers can complete their order without back-and-forth messages', '✓', '✓'], ['Order Confirmation', 'Automatically confirm every purchase', '✓', '✓'], ['Email Order Tracking', 'Customer receives a secure tracking link', '✓', '✓'], ['Business Assistant', 'Answers basic product & delivery questions', 'Advanced conversations', 'Advanced + personalised selling'], ['Customer Chat Inside Popup', '✓', '✓', '✓'], ['Smart Negotiation', '—', 'Let customers negotiate within your rules', 'Advanced negotiation & offers'], ['Product Recommendations', '—', 'Suggest related products to increase the order', 'Personalised upselling & cross-selling'], ['Abandoned Cart Recovery', '—', 'Bring interested customers back to complete purchases', 'Advanced recovery campaigns'], ['Campaign Creation', 'Create basic promotional posts', 'Create complete campaigns from your products', 'Create and automate multiple campaigns'], ['Social Selling Content', '✓', 'More creative variations', 'Advanced campaign creation'], ['Sales Analytics', 'See orders and revenue', 'See Views → Shops → Carts → Sales', 'Advanced conversion & revenue insights'], ['Customer Insights', 'Basic', 'Understand what customers buy and ask about', 'Segment and target customers'], ['Referral Selling', '—', '✓', 'Advanced referral & rewards'], ['Inventory Management', 'Basic', 'Advanced', 'Advanced'], ['AI Content Generation', 'Limited', 'More generations', 'High-volume generation'], ['Transaction Fee', '2% / successful order', '1% / successful order', '0.5% / successful order'],
  ];
  ...
  return <section id="pricing" className="bg-[#e9e6fa] px-5 py-24 lg:px-10 lg:py-32"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-8 md:flex-row md:items-end"><SectionIntro eyebrow="StatusSeller Pricing" title={<>Choose your<br /><span className="text-[#a25dd1]">selling advantage.</span></>} body="Every plan turns attention into a clearer path to chat, payment and orders. Start where you are, then grow into the next level." /><div className="flex flex-wrap items-center gap-3"><div className="flex items-center gap-1 rounded-full border border-[#192541]/15 bg-[#fbf9f3]/65 p-1 text-xs font-bold"><button onClick={() => setAnnual(false)} className={`rounded-full px-4 py-2 ${!annual ? 'bg-[#192541] text-[#f6f2e8]' : 'text-[#192541]/55'}`} data-testid="button-billing-monthly">Monthly</button><button onClick={() => setAnnual(true)} className={`rounded-full px-4 py-2 ${annual ? 'bg-[#16b878] text-[#192541]' : 'text-[#192541]/55'}`} data-testid="button-billing-annual">Annual · 2 months free</button></div><label className="flex items-center gap-2 rounded-full border border-[#192541]/15 bg-[#fbf9f3]/65 px-3 py-2 text-xs font-bold text-[#192541]">Currency<select value={currency} onChange={(event) => setCurrency(event.target.value as typeof currency)} className="rounded-md bg-transparent font-bold outline-none" aria-label="Choose currency" data-testid="select-pricing-currency"><option value="KES">KES</option><option value="USD">USD</option><option value="NGN">NGN</option><option value="ZAR">ZAR</option></select></label></div></div><div className="mt-10 overflow-x-auto rounded-[26px] border border-[#192541]/10 bg-[#fbf9f3] shadow-[0_18px_50px_rgba(25,37,65,.08)]"><table className="w-full min-w-[980px] border-collapse text-left"><thead><tr className="border-b border-[#192541]/10"><th className="w-[28%] p-5 align-bottom text-xs font-bold uppercase tracking-[.14em] text-[#192541]/50">Features</th>{planNames.map((name, i) => <th key={name} className="w-[24%] p-5 align-top"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.15em]" style={{ color: planColors[i] }}><span className="h-2 w-2 rounded-full" style={{ backgroundColor: planColors[i] }} />{name}{name === 'Growth' && <span className="rounded-full bg-[#f8bf3c] px-2 py-1 text-[9px] tracking-normal text-[#192541]">Popular</span>}</div><p className="mt-4 font-display text-3xl font-bold tracking-[-.05em] text-[#192541]">{money(currencies[currency].values[i])}<span className="ml-1 text-xs font-semibold tracking-normal text-[#192541]/45">/mo</span></p><p className="mt-2 min-h-[48px] max-w-[220px] text-xs font-medium leading-5 text-[#192541]/55">{planDescriptions[i]}</p><button onClick={() => onAction(`${name} plan selected — join the StatusSeller waitlist`)} className={`mt-4 w-full rounded-xl py-3 text-xs font-bold ${i === 1 ? 'bg-[#192541] text-[#f6f2e8]' : 'border-2 border-[#192541] text-[#192541]'}`} data-testid={`button-price-${name.toLowerCase()}`}>Join the waitlist <ArrowRight className="ml-1 inline h-3.5 w-3.5" /></button></th>)}</tr></thead><tbody>{features.map(([feature, starter, growth, pro], index) => <tr key={feature} className={`${index === 0 || index === 8 || index === 12 || index === 17 || index === 19 ? 'border-t-2 border-[#192541]/10' : 'border-t border-[#192541]/[.06]'} align-top`}><th className="p-4 text-sm font-bold text-[#192541]">{feature}</th>{[starter, growth, pro].map((value, i) => <td key={`${feature}-${i}`} className={`p-4 text-sm leading-5 ${value === '✓' ? 'font-bold text-[#16b878]' : value === '—' ? 'text-[#192541]/30' : 'text-[#192541]/62'}`}>{value}</td>)}</tr>)}</tbody></table></div><p className="mt-4 text-xs text-[#192541]/50">Prices shown in your selected currency. Annual billing is charged for 10 months and includes two months free. Transaction fees apply only to successful orders.</p></div></section>;
}
